<?php

return [
    'user_agent' => env('BIODIVERSITY_USER_AGENT', 'observations-api-audit/0.1 (local technical audit)'),
    'timeout_seconds' => (int) env('BIODIVERSITY_TIMEOUT_SECONDS', 15),
    'min_interval_ms' => (int) env('BIODIVERSITY_MIN_INTERVAL_MS', 500),
    'inaturalist_import_pause_ms' => (int) env('INATURALIST_IMPORT_PAUSE_MS', 0),
    'inaturalist_import_page_size' => (int) env('INATURALIST_IMPORT_PAGE_SIZE', 200),
    'geocoding_autocomplete_url' => env('GEOCODING_AUTOCOMPLETE_URL', 'https://data.geopf.fr/geocodage/completion/'),
    'administrative_geocoding_url' => env('ADMINISTRATIVE_GEOCODING_URL', 'https://geo.api.gouv.fr/communes'),
    'elevation_url' => env('ELEVATION_URL', 'https://data.geopf.fr/altimetrie/1.0/calcul/alti/rest/elevation.json'),
    'elevation_resource' => env('ELEVATION_RESOURCE', 'ign_rge_alti_wld'),
    'geography_enrichment_batch_size' => max(1, min(500, (int) env('GEOGRAPHY_ENRICHMENT_BATCH_SIZE', 50))),
    'retry_delay_multiplier' => (int) env('BIODIVERSITY_RETRY_DELAY_MULTIPLIER', 1),
    'import_limit_per_source' => min((int) env('BIODIVERSITY_IMPORT_LIMIT', 10000), 10000),
    'faune_france_token' => env('FAUNE_FRANCE_TOKEN'),
    'faune_france_bot_token' => env('FAUNE_FRANCE_BOT_TOKEN'),
    'faune_france_bot_stale_seconds' => (int) env('FAUNE_FRANCE_BOT_STALE_SECONDS', 300),
    'faune_france_max_pages' => (int) env('FAUNE_FRANCE_MAX_PAGES', 200),
    'faune_france_page_pause_ms' => (int) env('FAUNE_FRANCE_PAGE_PAUSE_MS', 0),
    'retention_days' => (int) env('BIODIVERSITY_RETENTION_DAYS', 365),
    'monitoring_history_months' => max(1, (int) env('MONITORING_HISTORY_MONTHS', 2)),
    'monitoring_overlap_minutes' => max(0, (int) env('MONITORING_OVERLAP_MINUTES', 10)),
    'map_observation_limit' => min(30000, max(1000, (int) env('MAP_OBSERVATION_LIMIT', 30000))),
    'inaturalist_gbif_dataset_key' => env('INATURALIST_GBIF_DATASET_KEY', '50c9509d-22c7-4a22-a47d-8c48425ef4a7'),
    'sources' => [
        'gbif' => ['base_url' => 'https://api.gbif.org/v1'],
        'inaturalist' => ['base_url' => 'https://api.inaturalist.org/v1'],
        'obis' => ['base_url' => 'https://api.obis.org/v3', 'enabled' => (bool) env('OBIS_ENABLED', false)],
        'ebird' => ['base_url' => 'https://api.ebird.org/v2', 'api_key' => env('EBIRD_API_KEY')],
        'taxref' => ['base_url' => 'https://taxref.mnhn.fr/api'],
        'geonature' => ['base_url' => env('GEONATURE_BASE_URL')],
    ],
];
